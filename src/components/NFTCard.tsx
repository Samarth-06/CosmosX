...
interface NFTCardProps {
  ...
  price: number;
  owner: string;
  /** Ownership / listing status badge (e.g. "Owned by me", "Not minted"). */
  statusLabel?: string;
  /** When false, Buy is disabled and shows buyDisabledReason. */
  canBuy?: boolean;
...
export default function NFTCard({
  ...
  const buyEnabled = canBuy && !isUserOwned;
  ...
  return (
    ...
    >
      {/* Atmosphere radial shine on hover */}
      <div
      ...
      >
        ...
        >
          <Heart
            className={`h-3.5 w-3.5 ${isWatchlisted ? "fill-rose-500 text-rose-500" : "text-white/60 hover:text-white"}`}
          />
        </button>
      ...
      <div className="flex flex-1 flex-col justify-between p-4">
        <div className="space-y-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="font-mono text-[9px] text-muted-foreground bg-white/5 rounded px-1.5 py-0.5">
          ...
          <div className="grid grid-cols-3 gap-1 py-1.5 border-y border-white/5 font-mono text-[8px] text-muted-foreground text-center">
            <div className="flex flex-col border-r border-white/5">
              <span className="text-[10px] text-white/80 font-bold">{planet.distance.toLocaleString()} ly</span>
              <span>Distance</span>
            </div>
            <div className="flex flex-col border-r border-white/5">
              <span className="text-[10px] text-white/80 font-bold">{planet.radius} {planet.radiusUnit}</span>
              <span>Radius</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-white/80 font-bold">{planet.temperature} K</span>
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-3 mt-1 text-left">
          ...
          <div className="text-right">
            ...
            <div className="font-mono text-[10px] text-white/80 font-semibold">
              {isUserOwned ? "YOU" : owner === "—" || owner === "Not minted" ? "—" : owner.slice(0, 6) + "..." + owner.slice(-3)}
            </div>
            {statusLabel && (
              <div className="font-mono text-[8px] text-indigo-300/90 mt-0.5">{statusLabel}</div>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-2 mt-3 pt-1">
          {isUserOwned ? (
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.03] py-2 text-[10px] font-semibold text-muted-foreground cursor-not-allowed"
              title={buyDisabledReason ?? undefined}
            >
              {buyDisabledReason ?? "Unavailable"}
            </button>
          )}
